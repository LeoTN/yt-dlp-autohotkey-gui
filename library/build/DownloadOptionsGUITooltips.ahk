...
onInit() {
    ...
    if (!A_Args.Has(1)) {
        MsgBox("This is a sub script from VideoDownloader which can only be called with specific parameters. "
            . "It is responsible for showing tooltips to the user.`n`nHave a nice day :)", "VD - Tooltip Manager",
            "O Iconi T6")
        ExitApp()
    }
...
handleDownloadOptionsGUI_toolTipLoop() {
    ...
    if (!elementToolTipArray.Has(2)) {
        ...
        tmp4 :=
            "Hides the PowerShell status window and disables all script notifications concerning the download process."
        ...
        tmp9 :=
            "Thumbnails and subtitles will be embedded into the media file. Not every video format supports embedding!"
        ...
        tmp19 :=
            "Recodes the video into another format using FFmpeg, which will dramatically increase processing time, but allows "
            .
        ...
        tmp23 :=
            "Here you can put your own parameters.`nIt is not recommended to leave space after the last parameter."
        tmp24 := "Allows you to change the download folder`nindependently from the config file."
        tmp25 :=
            "Keep in mind, that selecting a folder will actually download straight into it without any timestamp subfolders."
